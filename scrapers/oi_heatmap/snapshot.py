...
SNAPSHOTS_DIR = Path(__file__).parent.parent.parent / "data" / "snapshots" / "oi_heatmap"


def save_snapshot(product: str, all_tabs: dict) -> Path:
    """Save all tabs as a timestamped JSON. Skip if data unchanged."""
    latest = _latest_snapshot(product)
    if latest:
    ...
    snap_dir = SNAPSHOTS_DIR / product
    ...
    print(f"  Saved: {out.relative_to(snap_dir.parent.parent.parent)}")
    return out


def _latest_snapshot(product: str) -> Optional[Path]:
    snap_dir = SNAPSHOTS_DIR / product
